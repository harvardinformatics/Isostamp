...
def database_read_transform(target_csv_path, decoy_csv_path):
    ...
    psmdf['fragment_pattern_score']=psmdf['xcorr']
    return psmdf
...
def propensity_from_first_scan_v2(psmdf, ms1df):
    ...
    df['fragment_pattern_score']=np.exp((df['fragment_pattern_score']/df['charge'])+(df['isostamp_propensity']+df['regular_propensity']+10*df['base_peak_correlation'])/3)
    df.loc[df['rt_min'] >= df['rt_min'].quantile(.9), 'fragment_pattern_score'] = 0
    df.loc[df['base_peak_correlation'] <= 0.5, 'fragment_pattern_score'] = 0
